...
import backgroundConfig from './beautifier-backgroundConfig';
...
export const getBackground = (key: string) => {
    ...
    if (backgroundConfig[key as keyof typeof backgroundConfig]) return backgroundConfig[key as keyof typeof backgroundConfig];
    return 'bg-white';
};
...
export const fileToDataURL = (file: File): Promise<{ src: string; img: HTMLImageElement }> =>
    new Promise((resolve, reject) => {
        const reader = new FileReader();
        reader.onload = function (e) {
            const img = new Image();
            img.onload = function () {
                resolve({ src: e.target?.result as string, img });
            };
...
export const getImgColor = async (img: string) => {
    let colors: number[][] = [];
    let width = 0;
    let height = 0;
    ...
    try {
        ...
        centroids.map((e: number[]) => {
            if (e.some((i: number) => isNaN(i))) return e;
            colors.push(e.map((i: number) => parseInt(i.toString())));
        });
...
export const scatterArray = (arr: number[][]) => {
        ...
        b.push(
            `url("data:image/svg+xml,${encodeURIComponent(
                `<svg xmlns="http://www.w3.org/2000/svg" width="100%" height="100%" viewBox="0 0 1440 900" fill="none" style="background: rgb(${arr[
                    ...
                    i
                ].join(',')})" cx="1319.7" cy="799.3" rx="556.5" ry="379" transform="rotate(22 1319.7 799.3)" style="filter: blur(200px);"></ellipse></svg>`
            )}")`
        );
        c.push(
            `url("data:image/svg+xml,${encodeURIComponent(
...
export const captureScreen = async (): Promise<string | undefined> => {
    try {
        ...
        await new Promise((resolve) => {
            video.onplaying = resolve as () => void;
        });
...
export const computedSize = (w: number, h: number, maxWidth = 950, maxHeight = 450) => {
    let width = w;
    let height = h;
