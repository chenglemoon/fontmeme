...
export default function useAnnotate() {
    ...
    const handleMouseDown = (e: any) => {
        ...
        const newShape: Shape = { id, type: tool, points: [pos.x, pos.y], color: annotateColor, strokeWidth: annotateWidth };
    ...
    const handleMouseMove = (e: any) => {
        ...
        if (['line', 'arrow'].includes(lastShape.type) && lastShape.points && lastShape.points.length >= 2) {
            lastShape.points = [lastShape.points[0], lastShape.points[1], point.x, point.y];
        }
        if (['rect', 'filledRect'].includes(lastShape.type) && lastShape.points && lastShape.points.length >= 2) {
            lastShape.width = point.x - lastShape.points[0];
            lastShape.height = point.y - lastShape.points[1];
        }
        if (lastShape.type === 'circle' && lastShape.points && lastShape.points.length >= 2) {
    ...
    const onAnnotateWidthChange = (e: number) => {
        setAnnotateWidth(e);
        if (selectedId) {
            const currentShaps = shapes.find(e => e.id === selectedId);
