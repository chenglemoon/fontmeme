...
export default function HeroSection() {
  const t = useTranslations("CocaColaFont.hero");
  const tGen = useTranslations("CocaColaFont.generator");
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const [text, setText] = useState("Coca-Cola");
  const [selectedFont, setSelectedFont] = useState(AVAILABLE_FONTS[0].name);
  const [fontSize, setFontSize] = useState([65]);
  const [color, setColor] = useState("#F40009"); // Default Coca-Cola red
