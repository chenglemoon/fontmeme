...
export default function HeroSection() {
  const t = useTranslations("MontserratFont.hero");
  const tGen = useTranslations("MontserratFont.generator");
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const [text, setText] = useState("MONTSERRAT");
  const [selectedFont, setSelectedFont] = useState(AVAILABLE_FONTS[2].name); // MontserratRegular
