...
export default function HeroSection() {
  const t = useTranslations("SonicFont.hero");
  const tGen = useTranslations("SonicFont.generator");
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const [text, setText] = useState("SONIC");
  const [selectedFont, setSelectedFont] = useState(AVAILABLE_FONTS[0].name); // NiseSegaSonic
  const [fontSize, setFontSize] = useState([65]);
  const [color, setColor] = useState("#0066FF"); // Default blue (Sonic blue)
  const [secondColor, setSecondColor] = useState("#FFD700"); // gold
