...
export default function HeroSection() {
  const t = useTranslations("GtaFont.hero");
  const tGen = useTranslations("GtaFont.generator");
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const [text, setText] = useState("GRAND THEFT AUTO");
  const [selectedFont, setSelectedFont] = useState(AVAILABLE_FONTS[0].name);
  const [fontSize, setFontSize] = useState([65]);
  const [color, setColor] = useState("#10b981"); // Default emerald (GTA theme)
