...
export default function HeroSection() {
  const t = useTranslations("SquidGameFont.hero");
  const tGen = useTranslations("SquidGameFont.generator");
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const [text, setText] = useState("SQUID GAME");
  const [selectedFont, setSelectedFont] = useState(AVAILABLE_FONTS[0].name);
  const [fontSize, setFontSize] = useState([65]);
  const [color, setColor] = useState("#FF0000"); // Default red (Squid Game theme)
