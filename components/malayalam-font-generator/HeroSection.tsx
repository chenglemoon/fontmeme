...
export default function HeroSection() {
  const t = useTranslations("MalayalamFontGenerator.hero");
  const tGen = useTranslations("MalayalamFontGenerator.generator");
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const [text, setText] = useState("മലയാളം");
  const [selectedFont, setSelectedFont] = useState(AVAILABLE_FONTS[0].name); // MLTTAmbiliNormal
