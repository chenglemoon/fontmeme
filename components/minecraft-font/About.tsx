...
export default function About() {
  const t = useTranslations("MinecraftFont.about");
  const [fontLoaded, setFontLoaded] = useState(false);
  const titleCanvasRef = useRef<HTMLCanvasElement>(null);
  const [selectedFont] = useState("Minecraft"); // 使用 Minecraft 字体
        ...
        >
          <div className="space-y-6">
            <div className="relative">
              ...
              <p 
                className="text-white text-base md:text-lg leading-relaxed pl-6"
                style={fontLoaded ? { fontFamily: `"${selectedFont}", sans-serif` } : {}}
              >
                {t("description1")}
              </p>
            </div>
