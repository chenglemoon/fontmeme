...
export default function HeroSection() {
  const t = useTranslations("TheLifeOfAShowgirlFont.hero");
  const tGen = useTranslations("TheLifeOfAShowgirlFont.generator");
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const [text, setText] = useState("The Life of a Showgirl");
  const [selectedFont, setSelectedFont] = useState(AVAILABLE_FONTS[0].name);
  const [fontSize, setFontSize] = useState([65]);
  const [color, setColor] = useState("#ffcc03"); // Default yellow
  ...
  const handleDownload = async () => {
    if (canvasRef.current) {
      await downloadCanvas(
        canvasRef.current, 
        `the-life-of-a-showgirl-font-${Date.now()}.png`,
        canvasWidth,
        canvasHeight
      );
    }
