...
export default function About() {
  const t = useTranslations("PokemonFont.about");
  const [fontLoaded, setFontLoaded] = useState(false);
  const titleCanvasRef = useRef<HTMLCanvasElement>(null);
  const [selectedFont] = useState("PokemonSolid"); // 使用 Pokemon 字体
  ...
  useEffect(() => {
    ...
    gradient.addColorStop(0, "#ffcc03");
    gradient.addColorStop(0.5, "#ffd700");
    gradient.addColorStop(1, "#ffcc03");
    ...
    <section className="py-16 px-4 bg-black relative overflow-hidden">
      ...
      <div className="container mx-auto max-w-5xl relative z-10">
        ...
        <motion.div
          ...
          className="bg-gradient-to-br from-neutral-900/90 via-neutral-900/80 to-neutral-800/90 rounded-2xl p-8 md:p-12 border border-yellow-500/20 shadow-2xl backdrop-blur-sm"
        >
          <div className="space-y-6">
            <div className="relative">
              <div className="absolute -left-4 top-0 bottom-0 w-1 bg-gradient-to-b from-yellow-400 to-yellow-600 rounded-full"></div>
              <p 
                className="text-white text-xl md:text-2xl leading-relaxed pl-6 font-medium"
                style={fontLoaded ? { fontFamily: `"${selectedFont}", sans-serif`, fontSize: '1.5rem', lineHeight: '2rem' } : {}}
              >
                <em className="text-yellow-400">Pokémon</em> {t("description1")}
              </p>
            </div>
