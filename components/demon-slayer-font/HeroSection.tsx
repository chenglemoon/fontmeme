...
export default function HeroSection() {
  const t = useTranslations("DemonSlayerFont.hero");
  const tGen = useTranslations("DemonSlayerFont.generator");
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const [text, setText] = useState("DEMON SLAYER");
  const [selectedFont, setSelectedFont] = useState(AVAILABLE_FONTS[0].name);
  const [fontSize, setFontSize] = useState([65]);
  const [color, setColor] = useState("#DC2626"); // Default red (Demon Slayer theme)
  const [secondColor, setSecondColor] = useState("#F59E0B"); // amber-500
  ...
  const handleDownload = async () => {
    if (canvasRef.current) {
      await downloadCanvas(
        canvasRef.current, 
        `demon-slayer-font-${Date.now()}.png`,
        canvasWidth,
        canvasHeight
      );
    }
