...
import { Download, RefreshCw, WandSparkles, Copy, Check } from "lucide-react";
import { AVAILABLE_FONTS, loadFont, preloadFonts } from "./utils/fontLoader";
import { renderText, downloadCanvas, canvasToBlob, type RenderOptions } from "./utils/canvasRenderer";
...
export default function HeroSection() {
  const t = useTranslations("CarlyRaeJepsenFont.hero");
  const tGen = useTranslations("CarlyRaeJepsenFont.generator");
  ...
  const [color, setColor] = useState("#10b981"); // emerald-500, 与标题颜色一致
  const [secondColor, setSecondColor] = useState("#14b8a6"); // teal-500, 与标题渐变一致
  ...
  const handleDownload = async () => {
    if (canvasRef.current) {
      // 始终使用输入框中显示的尺寸（如果已设置）
      // 如果未启用自定义尺寸，使用当前显示的尺寸值
      await downloadCanvas(
        canvasRef.current, 
        `carly-rae-jepsen-font-${Date.now()}.png`,
        canvasWidth,
        canvasHeight
      );
    }
  ...
  const handleCopyText = async () => {
    ...
    try {
      // 获取要复制的canvas（使用自定义尺寸或预览尺寸）
      let canvasToCopy = canvasRef.current;
      ...
      if (canvasWidth && canvasHeight) {
        ...
        if (ctx) {
          ...
          // 缩放并绘制原canvas内容到中心
          const scaleX = canvasWidth / canvasRef.current.width;
          const scaleY = canvasHeight / canvasRef.current.height;
  ...
  return (
    <main id="hero" className="overflow-hidden bg-black">
            ...
            <div className="relative mx-auto overflow-hidden rounded-3xl bg-transparent backdrop-blur-xl p-4 sm:p-6 shadow-xl shadow-gray-200/5 dark:shadow-black/5">
              <div className="space-y-6">
                ...
                <div className="hidden lg:block relative">
                  <div className="bg-[#1a1a1a]/40 rounded-xl p-4 space-y-4">
                    {/* First Row */}
                    ...
                    <div className="flex items-center gap-4">
                      <div className="flex items-center gap-3">
                        <Switch
                          ...
                          className="data-[state=checked]:bg-[var(--brand-success)]"
                        />
                        <Label className="font-medium text-sm cursor-pointer select-none text-white/90">
                      ...
                      {useCustomSize && (
                        <>
                          <div className="flex items-center gap-2">
                            <Label className="font-medium text-sm text-white/90 whitespace-nowrap">
                              {tGen("widthLabel")}
                            </Label>
                            <Input
                              type="number"
                              ...
                              onChange={(e) => {
                                ...
                                setCanvasWidth(isNaN(val) ? undefined : Math.max(100, Math.min(5000, val)));
                              }}
                              placeholder="800"
                              min={100}
                              max={5000}
                              className="w-24 bg-[#1a1a1a]/70 border-gray-600 text-white"
                            />
                            <span className="text-sm text-white/70">px</span>
                          </div>
                          <div className="flex items-center gap-2">
                            <Label className="font-medium text-sm text-white/90 whitespace-nowrap">
                              {tGen("heightLabel")}
                            ...
                            <Input
                              ...
                              onChange={(e) => {
                                const val = parseInt(e.target.value);
                                setCanvasHeight(isNaN(val) ? undefined : Math.max(100, Math.min(5000, val)));
                              }}
                              placeholder="600"
                              min={100}
                              max={5000}
                    ...
                    {/* Second Row - Font Size with full width slider */}
                    <div className="flex items-center gap-4">
                      <Label className="font-medium text-sm whitespace-nowrap text-white/90 shrink-0">
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <div className="flex items-center gap-2">
                          ...
                          <Input
                            ...
                            min={100}
                            max={5000}
                            className="flex-1 bg-[#1a1a1a]/70 border-gray-600 text-white"
                          />
                          <span className="text-sm text-white/70 shrink-0">px</span>
                        </div>
                        <div className="flex items-center gap-2">
                          <Label className="font-medium text-sm text-white/90 whitespace-nowrap shrink-0">
                            {tGen("heightLabel")}
